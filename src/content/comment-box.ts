...
function writeToNativeInput(input: HTMLTextAreaElement | HTMLInputElement, text: string): void {
...
function writeToContentEditable(input: HTMLElement, text: string): void {
  ...
  if (!inserted || normalizeText(readInputText(input)) !== normalizeText(text)) {
...
async function insertCommentText(input: HTMLElement, text: string): Promise<boolean> {
  if (input instanceof HTMLTextAreaElement || input instanceof HTMLInputElement) {
    input.focus({ preventScroll: true })
  ...
  if (normalizeText(readInputText(input)).includes(normalizeText(text))) return true
  ...
  variants.forEach((variant) => {
    ...
    card.addEventListener("click", async () => {
      ...
      if (!input) {
        ...
        status.textContent = "Could not find LinkedIn's comment box. Open it manually and try again."
      ...
      if (!inserted) {
        ...
        status.textContent = "LinkedIn rejected the inserted text. Click inside the comment box and try again."
        status.classList.add("lcg-panel-status-error")
        return
      }
