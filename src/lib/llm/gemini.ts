...
function validateVariants(value: unknown): CommentVariant[] {
  if (!Array.isArray(value) || value.length !== 3) {
    throw new LlmProviderError("INVALID_RESPONSE", "Gemini returned an unexpected response shape.")
  }

  const variants = value.map((item) => {
    if (!item || typeof item !== "object") {
      throw new LlmProviderError("INVALID_RESPONSE", "Gemini returned an invalid comment variant.")
    }
  ...
  if (new Set(variants.map((variant) => variant.tone)).size !== 3) {
    throw new LlmProviderError("INVALID_RESPONSE", "Gemini did not return all three requested tones.")
  }
...
export async function generateWithGemini(
  ...
  try {
    ...
    throw new LlmProviderError("NETWORK_ERROR", "Could not reach Gemini. Check your connection and try again.")
  }
  ...
  try {
    ...
    throw new LlmProviderError("INVALID_RESPONSE", "Gemini returned a response that could not be read.")
  }
