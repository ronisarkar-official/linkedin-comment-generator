...
import type { CommentRequest, CommentVariant, Tone, UserSettings } from "../types"
...
function validateVariants(value: unknown): CommentVariant[] {
  if (!Array.isArray(value) || value.length !== 3) {
    throw new LlmProviderError("INVALID_RESPONSE", "OpenRouter returned an unexpected response shape.")
  }

  const variants = value.map((item) => {
    if (!item || typeof item !== "object") {
      throw new LlmProviderError("INVALID_RESPONSE", "OpenRouter returned an invalid comment variant.")
    }
  ...
  if (new Set(variants.map((variant) => variant.tone)).size !== 3) {
    throw new LlmProviderError("INVALID_RESPONSE", "OpenRouter did not return all three requested tones.")
  }
...
function parseContent(content: string): CommentVariant[] {
      ...
      try {
        ...
        if (parsed && typeof parsed === "object") {
          ...
          for (const key of ["variants", "comments", "data", "result"]) {
            if (Array.isArray(record[key])) return validateVariants(record[key])
          }
        }
      } catch {
        continue
  ...
  for (const match of normalized.matchAll(labelPattern)) {
    const text = match[2]
      ...
      .replace(/^[-*]\s*/, "")
    if (text) {
      labelledVariants.push({ tone: match[1].toLowerCase() as Tone, text })
    }
  }

  if (labelledVariants.length === 3) return validateVariants(labelledVariants)

  throw new LlmProviderError("INVALID_RESPONSE", "OpenRouter returned a response that could not be parsed.")
}
...
export async function generateWithOpenRouter(
  ...
  try {
    ...
    throw new LlmProviderError("INVALID_RESPONSE", "OpenRouter returned a response that could not be read.")
  }

  if (!response.ok) {
    const message = payload.error?.message ?? "OpenRouter rejected the request."
    if (response.status === 401 || response.status === 403) {
      throw new LlmProviderError("INVALID_API_KEY", message)
    }
    if (response.status === 429) {
      throw new LlmProviderError("RATE_LIMITED", message, parseRetryAfter(response))
    }
  ...
  if (!content) {
    throw new LlmProviderError("INVALID_RESPONSE", "OpenRouter returned no comment variants.")
  }
