...
import type { CommentLength, LlmProvider, Tone, UserSettings } from "../lib/types"
...
export default function App() {
  ...
  const updateProvider = (provider: LlmProvider) => {
    setSettings((current) => ({ ...current, provider, apiKey: "" }))
    setStatus("")
  }
    ...
    <main className="w-[360px] bg-slate-50 text-slate-900">
      <header className="bg-gradient-to-br from-blue-700 to-blue-500 px-5 py-5 text-white">
        <div className="flex items-center gap-3">
          <img src="/icons/icon48.png" alt="" className="h-10 w-10 rounded-xl shadow-sm" />
          <div>
            <h1 className="text-lg font-bold leading-tight">LinkedIn Comment Generator</h1>
            <p className="mt-0.5 text-xs text-blue-100">
              {settings.provider === "openrouter" ? "OpenRouter Free Router" : "Gemini 2.5 Flash"}
            </p>
          </div>
        </div>
      </header>
        ...
        {loading ? (
          ...
          <>
            <fieldset className="space-y-2">
              <legend className="text-sm font-semibold text-slate-800">AI provider</legend>
            ...
            <ToneSelector value={settings.defaultTone} onChange={updateTone} />
