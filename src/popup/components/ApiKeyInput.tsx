...
export default function ApiKeyInput({ value, provider, onSave }: ApiKeyInputProps) {
  ...
  const handleSave = async () => {
    ...
    if (!isValid) {
      setIsError(true)
      setFeedback(`Enter a valid ${provider === "openrouter" ? "OpenRouter" : "Gemini"} API key.`)
      return
    }
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="text-sm font-semibold text-slate-800" htmlFor="api-key">
