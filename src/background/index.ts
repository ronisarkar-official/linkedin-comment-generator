...
function isGenerateMessage(value: unknown): value is GenerateCommentsMessage {
  ...
  return message.action === "GENERATE_COMMENTS" && isCommentRequest(message.payload)
}
...
async function handleGenerate(request: CommentRequest): Promise<GenerateCommentsResponse> {
  try {
    const settings = await getSettings()
    const variants = await generateComments(request, settings)
...
chrome.runtime.onMessage.addListener((message: unknown, _sender, sendResponse) => {
  ...
  void handleGenerate(message.payload).then(sendResponse).catch((error: unknown) => {
